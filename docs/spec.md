...
Spring Boot(백엔드) + Next.js 16(프론트엔드) + Supabase PostgreSQL(DB)로 구성된 배포형 모노레포 앱.
...
| 차트 | react-chartjs-2 + chart.js, Recharts | - |
| 플래너 캘린더 | FullCalendar (timeGridDay + interaction) | - |
...
│   ├── schedule    (TodoSchedule, TodoScheduleRepository, TodoScheduleService, DefaultTodoScheduleService, TodoScheduleController)
...
> `schedule` 도메인은 CQRS 없이 단일 `TodoScheduleService` 인터페이스 + `DefaultTodoScheduleService`로 구성 (단순 CRUD).
...
- `todos` : `todo_schedules` = 1 : 0..1 (todo_id FK, UNIQUE — Todo당 최대 1개 슬롯)
...
|------|------|------|------|
| id | BIGSERIAL | PK | |
| todo_id | BIGINT | FK → todos.id, UNIQUE | Todo당 1개 슬롯 보장 |
| start_time | TIME | NOT NULL | 시작 시각 (예: 10:00:00) |
| end_time | TIME | NOT NULL | 종료 시각 (예: 11:00:00) |
| schedule_date | DATE | NOT NULL | 플래너 기준 날짜. 자정 이후 슬롯도 "어느 날 플래너"인지 특정하기 위해 별도 관리 |
...
#### GET /api/todo-schedules?date={YYYY-MM-DD}
해당 날짜의 모든 스케줄 목록 반환.
**200** `ScheduleResponse[]`

#### POST /api/todo-schedules
```json
{
  "todoId": 42,
  "startTime": "10:00:00",
...
```
**201** 생성된 `ScheduleResponse` / **409** 이미 해당 Todo에 스케줄 존재

#### PUT /api/todo-schedules/{id}
```json
{ "startTime": "14:00:00", "endTime": "15:30:00" }
```
**200** 수정된 `ScheduleResponse` / **404** 없음

#### DELETE /api/todo-schedules/{id}
**204** No Content / **404** 없음

**ScheduleResponse 형태**
```json
{ "id": 1, "todoId": 42, "startTime": "10:00:00", "endTime": "11:00:00", "scheduleDate": "2026-06-03" }
```
...
#### GET /api/health
서버 상태 확인. 인증 불필요.
**200** `{ "success": true, "data": null, "message": null }`
...
| 401 | 미인증 (세션 없음) |
| 404 | 리소스 없음 |
| 409 | 리소스 충돌 (예: 이미 스케줄이 존재하는 Todo에 중복 등록) |
| 500 | 서버 내부 오류 |

- `@ControllerAdvice` 전역 예외 핸들러
- `MethodArgumentNotValidException` → 400
- `EntityNotFoundException` → 404
- `DataIntegrityViolationException` (UNIQUE 위반 등) → 409
